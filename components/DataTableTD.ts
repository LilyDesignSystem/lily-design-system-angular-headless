 ...
 * the consumer writes `<td lily-data-table-td>`, so the host element IS the
 ...
 * `data-table-td` plus the consumer-provided `className` on the host. Ships
 * zero CSS; the consumer styles via the class hook.
 */
@Component({
  selector: "td[lily-data-table-td]",
  standalone: true,
  template: `<ng-content />`,
  host: {
