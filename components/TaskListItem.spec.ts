...
// TaskListItem is an attribute-selector component (`li[lily-task-list-item]`): its host is the
// native <li> the consumer writes, not a wrapper element, so tests mount it via a
// small host template rather than `TestBed.createComponent(TaskListItem)` directly (the
...
describe("TaskListItem", () => {
  test("the host is the li itself — no wrapper element — with the base class", () => {
    ...
    expect(el.classList.contains("task-list-item")).toBe(true);
  });
