...
// PaginationListItem is an attribute-selector component (`li[lily-pagination-list-item]`): its host is the
// native <li> the consumer writes, not a wrapper element, so tests mount it via a
// small host template rather than `TestBed.createComponent(PaginationListItem)` directly (the
// latter has no <li> to attach to and falls back to a bare <div>).
@Component({
  standalone: true,
  imports: [PaginationListItem],
...
describe("PaginationListItem", () => {
  test("the host is the li itself — no wrapper element — with the base class", () => {
    ...
    expect(el.classList.contains("pagination-list-item")).toBe(true);
  });
