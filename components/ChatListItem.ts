 ...
 * the consumer writes `<li lily-chat-list-item>`, so the host element IS the
 ...
 * `chat-list-item` plus the consumer-provided `className` on the host. Ships
 * zero CSS; the consumer styles via the class hook.
 */
@Component({
  selector: "li[lily-chat-list-item]",
  standalone: true,
  template: `<ng-content />`,
  host: {
    "[class]": '"chat-list-item " + className()',
  },
