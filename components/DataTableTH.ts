 ...
 * the consumer writes `<th lily-data-table-th>`, so the host element IS the
 ...
 * `data-table-th` plus the consumer-provided `className` on the host. Ships
 * zero CSS; the consumer styles via the class hook.
 */
@Component({
  selector: "th[lily-data-table-th]",
  standalone: true,
  template: `<ng-content />`,
  host: {
    "[class]": '"data-table-th " + className()',
