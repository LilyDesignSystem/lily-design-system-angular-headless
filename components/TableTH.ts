 ...
 * the consumer writes `<th lily-table-th>`, so the host element IS the
 ...
 * `table-th` plus the consumer-provided `className` on the host. Ships
 * zero CSS; the consumer styles via the class hook.
 */
@Component({
  selector: "th[lily-table-th]",
  standalone: true,
  template: `<ng-content />`,
  host: {
    "[class]": '"table-th " + className()',
