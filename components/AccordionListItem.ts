 ...
 * the consumer writes `<li lily-accordion-list-item>`, so the host element IS the
 ...
 * `accordion-list-item` plus the consumer-provided `className` on the host. Ships
 * zero CSS; the consumer styles via the class hook.
 */
@Component({
  selector: "li[lily-accordion-list-item]",
  standalone: true,
  template: `<ng-content />`,
  host: {
