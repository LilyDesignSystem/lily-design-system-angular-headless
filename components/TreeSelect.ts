...
@Component({
  ...
  standalone: true,
  template: `<select class="tree-select {{ className() }}" [attr.aria-label]="label() || null" [value]="value()" (change)="value.set(($event.target as HTMLSelectElement).value)"><ng-content /></select>`,
