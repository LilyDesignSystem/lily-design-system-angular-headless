 ...
 * the consumer writes `<li lily-check-list-item>`, so the host element IS the
 ...
 * `check-list-item` plus the consumer-provided `className` on the host. Ships
 * zero CSS; the consumer styles via the class hook.
 */
@Component({
  selector: "li[lily-check-list-item]",
  standalone: true,
  template: `<ng-content />`,
  host: {
    "[class]": '"check-list-item " + className()',
  },
